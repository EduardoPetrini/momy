      ...
      fields: Object.keys(collections[name])
        ...
        .map(fieldName => {
          ...
          return {
            ...
            primary: false // set primary for 'id' or '_id'
            //primary: /_id$/.test(fieldName) // set primary for 'id' or '_id'
          }
        })
    }
  })
...
function convertCase(str, fieldCase) {
  return str === '_id' ? str :
    str.indexOf('._') >= 0 ? str.replace("._", "__") :
      fieldCase === 'camel' ? changeCase.camelCase(str) :
